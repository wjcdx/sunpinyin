...
CShuangpinSegmentor::pop()
{
    ...
    if (size == 1 || m_segs.back().m_type != INVALID) {
        m_hasInvalid = false;
    }
    if (l == 1)
    ...
    if (isComplete) {
        ...
        for (; iter != iter_end; ++iter) {
            ...
            if ((int)syl != 0) {
                ...
                m_segs.push_back(TStringSegment(ch, s.m_start, 1));
            }
        }
        return s.m_start;
    }
...
CShuangpinSegmentor::_push(unsigned ch)
{
    ...
    if (m_hasInvalid) {
        startFrom = len - 1;
        m_segs.push_back(TInvalidSegment(ch, startFrom, 1));
        goto RETURN;
    }
    ...
    if (!isInputPy) {
        ...
        if (ch == '\'' && m_inputBuf.size() > 1)
            m_segs.push_back(TSeperatorSegment(ch, startFrom, 1));
        else
            m_segs.push_back(TStringSegment(ch, startFrom, 1));
