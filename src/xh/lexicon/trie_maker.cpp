...
CTrieMaker::CNodeList CTrieMaker::TNode::m_AllNodes;
CTrieMaker::TNode::TNode()
{
    m_AllNodes.push_back(this);
}

bool
CTrieMaker::PNodeSet::operator<(const PNodeSet& another) const
{
...
parseUnit(const char *unit, std::vector<TUnit> &ret)
{
    ...
    while (*p) {
        if (*p == '\'') {
            *p = '\0';
            unsigned s = CEncodeData::encodeUnit(q);
            if (s)
